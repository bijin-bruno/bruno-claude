...
- **suggestion** — readability problems from the rule: unclear or misleading names, premature reuse
  abstraction, single-line indirection, `?.` where the null case isn't handled right there, needless
  whitespace/diff churn, or missing comments on genuinely complex flow.
