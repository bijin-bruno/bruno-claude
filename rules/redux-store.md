...
(~19 keys); read it rather than trusting any list here. Middleware is
...
Two gotchas:
- **Reducer key ≠ folder/file name.** e.g. reducer key `devtools` comes from `slices/devUtils/`;
  `globalEnvironments` from `slices/global-environments.js`; `openapiSync` from
  `slices/openapi-sync.js`.
...
This is the **superset** repo. Paid-only slices (absent from the open-source sibling) include
`git/`, `license`, `trial`, `github`, `gitProviders`, `importTracker`, `autoUpdater`, `devUtils`,
and the `git` middleware. Don't assume a slice exists in both repos — check before relying on it.
...
Six middlewares, registered in `index.js` (read the directory + registration array for the live
set):
...
- **`git`** (paid-only) — listener middleware reacting to tab/git-sync actions.
...
used in just a couple of places (`git/selectors.js`, `src/selectors/tab.js`). Default to inline
...
`ReduxStore/`, `Theme/`, `Hotkeys/`, `Toaster/`, `PromptVariables/`, plus paid-only `LicenseGuard/`
(premium gating) and `Dictionary/` (i18n).
