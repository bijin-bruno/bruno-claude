...
- **`.claude/CLAUDE.md`** loads every session — it is a first-class project-instruction location,
  so **no root `CLAUDE.md` and no `@` import are needed**. (Bruno's root `.gitignore` excludes both
  `.claude/` and a root `CLAUDE.md`, so shared guidance must live here.)
...
```bash
cd path/to/bruno/.claude && git pull
```
...
**Why no nested per-package `CLAUDE.md`?** This config is a standalone repo cloned into a gitignored
`.claude/`; it can't ship files into Bruno's package directories. Central path-scoped rules are the
correct tool here (see the Claude Code "large codebases" guide) and keep all conventions in one
maintained place.
...
Owned by `@bijin-bruno` (see `.github/CODEOWNERS`). Improvements to rules and skills are welcome via
PR; each rule/skill's author is expected to keep it verified against the current Bruno source.
